...
public class UnitCommand : MonoBehaviour
{
        ...
        if (Physics.Raycast(ray, out hit, 1000, layerMask))
        {
            switch (hit.collider.tag)
            {
                case "Ground":
                    ...
                    break;
                case "Resource":
                    ResourceCommand(hit, unitSelect.CurUnit);  //1
                    break;
                case "Unit":
    ...
    private void UnitsToGatherResource(ResourceSource resource, List <Unit> unit)
    {
    ...
    private void ResourceCommand(RaycastHit hit, List<Unit> unit)
    {
